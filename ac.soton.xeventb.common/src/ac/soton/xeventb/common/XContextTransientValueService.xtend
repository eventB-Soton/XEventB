...
class XContextTransientValueService extends DefaultTransientValueService {
	...
	override boolean isTransient(EObject owner, EStructuralFeature feature, int index) {
		...
		if (owner instanceof Context) {
			...
			// return false;
			if(feature.equals(ContextPackage.Literals.CONTEXT__EXTENDS)) return false
			if(feature.equals(ContextPackage.Literals.CONTEXT__SETS)) return false
			if(feature.equals(ContextPackage.Literals.CONTEXT__CONSTANTS)) return false
			if(feature.equals(ContextPackage.Literals.CONTEXT__AXIOMS)) return false
			return true
		}
		...
		if (owner instanceof Constant) {
			if(feature.equals(CorePackage.Literals.EVENT_BNAMED__NAME)) return false else if(feature.equals(
				CorePackage.Literals.EVENT_BCOMMENTED__COMMENT)) return true
			// return false;
			return true
		}
